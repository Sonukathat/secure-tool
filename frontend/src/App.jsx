...
function App() {
  ...
  const chartData = React.useMemo(() => {
    ...
    dailyResult.forEach(d => {
      const dateStr = new Date(d.date).toDateString(); // normalize date
    ...
    return Object.entries(grouped)
      .map(([date, total]) => ({
        date: new Date(date), // real JS Date object
        total: total / 1000000,
      }))
  ...
  const totalInMillion =
    dailyResult.reduce((acc, curr) => acc + (curr.totalCost || 0), 0) /
    1000000;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-purple-300 via-pink-200 to-yellow-200 p-8">
              ...
              {chartData.length > 0 && (
                <>
                  <ReferenceArea
                    y1={0}
                    y2={4}
                    x1={chartData[0].date.getTime()} // start of X-axis
                    x2={chartData[chartData.length - 1].date.getTime()} // end of X-axis
                    fill="green"
                    fillOpacity={0.1}
                  />
                  <ReferenceArea
      ...
      dataKey="total"
      stroke="#ff7300"
      strokeWidth={2} // line thickness (vertical)
    >
